...
var numbers = [2,43,5];
sum = numbers.reduce(function(sum, number){         // argument signature varies for reduce compared to other
  return number + sum;
...
console.log(primaryColors.reduce(function(prev, item){
...
output = !input_seq.split("").reduce(function(prev, item){
  ...
  if (prev < 0) {return prev; }       // case for when sequence becomes negative altogether
  if (item === '(' ) {return ++prev; } 
  else if (item === ')') {return --prev; }
...
deskTypes = desks.reduce(function(prev, desk) {
...
console.log(numbers.some(function(number){         // argument signature varies for reduce compared to other
  return number > 15;
...
output = chars.filter(function(char){
  return char > 0;
}).map(function(char){
  return char * 2;
});
